...
export default class ParticlesManager {
	...
	particleAmount: number = 100;
	defaultRadius: number = 2;
	variantRadius: number = 2;
	defaultSpeed: number = 1;
	variantSpeed: number = 5;
	maxLinkRadius: number = 300;
	linkRadius: number;
	minLinkRadius: number;
	backgroundColor: string;
	maxLineWidth: number = 0.5;
	minLineWidth: number = 0.05;
	lineWidth: number;
	rgb: string[];
	stop: boolean = false;
	t: number;
	...
	constructor({
		...
		this.canvasWidth = parseInt(canvas.style.width, 10) || canvas.width;
		this.canvasHeight = parseInt(canvas.style.height, 10) || canvas.height;
		this.particles = [];
