...
const check = async (toPut: Array<number>, i:number, data:Record<string, any>, trait_type:string, targetAttribute:string) => {
    ...
    for (const attribute of attributes) {
        if (attribute["trait_type"].toLowerCase() == trait_type && attribute["value"].toLowerCase() == targetAttribute) {
            toPut.push(i)
        }
    }
}
export default async (url:string, max_token:number, trait_type:string, attribute:string) => {
    url = url.endsWith("/") ? url : `${url}/`
    const toPut = [] as Array<number>
    const begin = 1
    const urls = Array.from({length: max_token}, (_, i) => fetch(`${url}${i+begin}`).then(r => r.json()).catch(() => {}));
