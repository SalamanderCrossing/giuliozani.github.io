...
class BirdGeometry extends THREE.BufferGeometry {
  constructor() {
    ...
    const references = new THREE.BufferAttribute(
      ...
      2,
    ); // the reference point for the bird wing (the wing is a triangle that rotates around this point)
    // it is a vec2 because the shader uses it as a 2 component vector
    // the first component is the index of the vertex in the geometry
    // the second component is the sign of the rotation (-1 or 1)
    ...
    for (let v = 0; v < triangles * 3; v++) {
      ...
      const x = (birdIndex % WIDTH) / WIDTH; // why not just birdIndex? because we want the same color for all 3 vertices of the same triangle.
...
function initComputeRenderer() {
  gpuCompute = new GPUComputationRenderer(WIDTH, WIDTH, renderer); // the WIDTH because we have a 2d texture
